...
class Blocks {
    ...
    public static function render_block(string $component, array $attributes): string {
        ...
        $theme_value = $merged_attributes['theme'] ?? '';
        if ($theme_value === '' || $theme_value === null) {
            if (!empty($shared_config['theme'])) {
                $merged_attributes['theme'] = $shared_config['theme'];
            }
        }
