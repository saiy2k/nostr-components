 ...
 * **[Post](#6-nostr-post)**
 * **[Livestream](#7-nostr-livestream)**
 * **[Fundraiser](#8-nostr-fundraiser)**
 * **[WordPress Integration](#9-wordpress-integration)**
...
All interactive actions (Follow, Like, Zap, and fundraiser zaps) require user authentication. Components use [window.nostr.js](https://github.com/fiatjaf/window.nostr.js) which supports:
...
![Preview of livestream](images/livestream-preview.png)

---

## 8. Nostr Fundraiser

Render NIP-75 fundraiser goals with a banner, fundraiser copy, progress bar, donor list, and event-targeted zap flow.
...
<head>
  <script
    type="module"
    src="https://cdn.jsdelivr.net/npm/nostr-components@latest/dist/components/nostr-fundraiser.es.js">
  </script>
</head>
<body>
  <nostr-fundraiser
    hex="YOUR_KIND_9041_EVENT_HEX"
...
- Provide exactly one identifier: `hex`, `noteid`, or `eventid`
- Reads required NIP-75 `amount` and `relays` tags from the fundraiser event
- Uses zap receipts (`kind: 9735`) linked to the fundraiser event id to compute progress
- In Storybook, use `Fundraiser > Manual Event Input` for a preview-first playground, `Fundraiser/Testing/Valid` for fixture-based happy paths, and `Fundraiser/Testing/Invalid` for identifier validation states
