...
export function parseLiveArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    } else if (flag === "--live-reconnect-min-ms") {
      args.liveReconnectMinMs = Number(take());
    } else if (flag === "--live-reconnect-max-ms") {
      args.liveReconnectMaxMs = Number(take());
...
function validateLiveArgs(args) {
  ...
  if (
    !Number.isFinite(args.liveReconnectMinMs) ||
    args.liveReconnectMinMs <= 0
  ) {
    throw new Error("--live-reconnect-min-ms must be positive.");
  }
  if (
    !Number.isFinite(args.liveReconnectMaxMs) ||
    args.liveReconnectMaxMs < args.liveReconnectMinMs
  ) {
    throw new Error(
      "--live-reconnect-max-ms must be >= --live-reconnect-min-ms.",
    );
  }
  if (
...
Options:
  ...
  --live-reconnect-min-ms <n>          Initial reconnect delay. Default: 1000
  --live-reconnect-max-ms <n>          Maximum reconnect delay. Default: 30000
  --live-seen-cache-limit <n>          In-memory dedupe size. Default: 50000
  --live-connect-timeout-ms <n>        WebSocket open timeout. Default: 15000
...
export function listenRelayLive(relay, args, callbacks) {
  const {
    signal,
    onConnectAttempt,
    onReconnect,
  ...
  return new Promise((resolve) => {
    ...
    const connect = () => {
      ...
      attempts += 1;
      onConnectAttempt?.();
      const subscriptionId = `nc-live-${Math.random().toString(36).slice(2, 10)}`;
